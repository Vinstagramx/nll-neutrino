...
for ind, val in enumerate(mix_ang_arr):
    prob_obj_m = NLL(energies = en_array, event_rates = event_no, mix_ang = val, distance = L, sq_mass_diff = sq_mass_diff)
    prob_m = prob_obj_m.surv_prob()
    angles = [val] * 200  # Mixing angle coordinate (y)
    
   # Assigning to coordinates array
    ...
    ang_coords[start_ind:end_ind, 2] = prob_m  # Assigning y-coordinates
...
# Varying the square-mass difference and generating a surface plot 
...
for ind, val in enumerate(sq_mass_arr):
    prob_obj_m = NLL(energies = en_array, event_rates = event_no, mix_ang = mix_ang, distance = L, sq_mass_diff = val)
    prob_m = prob_obj_m.surv_prob()
...
for ind, val in enumerate(dist_arr):
    prob_obj_m = NLL(energies = en_array, event_rates = event_no, mix_ang = mix_ang, distance = val, sq_mass_diff = sq_mass_diff)
    prob_m = prob_obj_m.surv_prob()
   ...
   # Assigning to coordinates array
    ...
    dist_coords[start_ind:end_ind, 2] = prob_m  # Assigning y-coordinates
